...
// Compatibility for non-Arduino environments
// #ifndef ARDUINO
...
int frameInterval = 10; // default value for 50 frames per second (1000/50 = 20 milliseconds)
...
// Close both eyes
void close() {
	eyeLheightNext = 6; // closing left eye
  eyeRheightNext = 6; // closing right eye
...
void close(bool left, bool right) {
  if(left){
    eyeLheightNext = 6; // blinking left eye
    eyeL_open = 0; // left eye not opened (=closed)
  }
  if(right){
      eyeRheightNext = 6; // blinking right eye
      eyeR_open = 0; // right eye not opened (=closed)
  }
}
...
void drawEyes(){
  ...
  // Open eyes again after closing them
	if(eyeL_open){
  	if(eyeLheightCurrent <= 6 + eyeLheightOffset){eyeLheightNext = eyeLheightDefault;}
  }
  if(eyeR_open){
  	if(eyeRheightCurrent <= 6 + eyeRheightOffset){eyeRheightNext = eyeRheightDefault;}
  }
