...
bool parseEmotion(const char* s, eEmotions &out) {
...
void setAutoMode(bool enable) {
    if (!gFace) return;
    gFace->RandomBehavior = enable;
    gFace->RandomLook = enable;
    gFace->RandomBlink = enable;
}
...
bool setEmotion(const char* emotionName) {
    if (!gFace) return false;
    eEmotions e;
    if (parseEmotion(emotionName, e)) {
...
bool setEmotionByIndex(int index) {
    if (!gFace) return false;
    if (index >= 0 && index < (int)eEmotions::EMOTIONS_COUNT) {
        gFace->RandomBehavior = false;
        gFace->Behavior.GoToEmotion((eEmotions)index);
        return true;
    }
    return false;
}
...
#ifdef _WIN32
void printEmotion(const char* name, const wchar_t* chinese) {
    HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
    CONSOLE_SCREEN_BUFFER_INFO csbi;
    GetConsoleScreenBufferInfo(hConsole, &csbi);

    DWORD written;
    const wchar_t* prefix = L"当前表情: ";
    WriteConsoleW(hConsole, prefix, wcslen(prefix), &written, NULL);

    char nameBuffer[32];
    sprintf_s(nameBuffer, "%s ", name);
    WriteConsoleA(hConsole, nameBuffer, strlen(nameBuffer), &written, NULL);
...
int main() {
    ...
    int emotionCount = sizeof(emotions) / sizeof(emotions[0]);
